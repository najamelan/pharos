...
async fn both()
{
	...
	let mut events: Events<IsisEvent> = isis.observe( Channel::Bounded( 5 ).into() ).await.expect( "observe" );
	let mut nuevts: Events<NutEvent>  = isis.observe( Channel::Bounded( 5 ).into() ).await.expect( "observe" );

	let mut ubevts: Events<IsisEvent> = isis.observe( ObserveConfig::default()     ).await.expect( "observe" );
	let mut ubnuts: Events<NutEvent>  = isis.observe( ObserveConfig::default()     ).await.expect( "observe" );
