...
async fn basic()
{
	...
	let mut events = isis.observe( Channel::Bounded( 2 ).into() ).await.expect( "observe" );
...
{
	let mut isis   = Goddess::new();
	let mut events = isis.observe( Channel::Bounded( 5 ).into() ).await.expect( "observe" );
...
#[ async_std::test ]
//
async fn one_receiver_drops()
{
	let mut isis       = Goddess::new();
	let mut egypt_evts = isis.observe( Channel::Bounded( 1 ).into() ).await.expect( "observe" );
	let mut shine_evts = isis.observe( Channel::Bounded( 2 ).into() ).await.expect( "observe" );
...
async fn types()
{
	...
	//
	let mut shine_evts: Events<NutEvent>  = isis.observe( Channel::Bounded( 5 ).into() ).await.expect( "observe" );
	let mut egypt_evts: Events<IsisEvent> = isis.observe( Channel::Bounded( 5 ).into() ).await.expect( "observe" );
...
{
	let mut isis       = Goddess::new();
	let mut egypt_evts = isis.observe( Channel::Bounded( 5 ).into() ).await.expect( "observe" );
	let mut shine_evts = isis.observe( Channel::Bounded( 5 ).into() ).await.expect( "observe" );
...
async fn filter()
{
	...
	let mut events = isis.observe( opts ).await.expect( "observe" );
