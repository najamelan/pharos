...
async fn basic()
{
	...
	let mut events = isis.observe( ObserveConfig::default() ).await.expect( "observe" );
...
async fn one_receiver_drops()
{
	...
	let mut egypt_evts = isis.observe( ObserveConfig::default() ).await.expect( "observe" );
	let mut shine_evts = isis.observe( ObserveConfig::default() ).await.expect( "observe" );
...
async fn types()
{
	...
	let mut egypt_evts: Events<IsisEvent> = isis.observe( ObserveConfig::default() ).await.expect( "observe" );
	let mut shine_evts: Events<NutEvent > = isis.observe( ObserveConfig::default() ).await.expect( "observe" );
...
#[ async_std::test ]
//
async fn alot_of_events()
{
	let mut w = Goddess::new();

	let mut events = w.observe( ObserveConfig::default() ).await.expect( "observe" );
...
async fn filter()
{
	...
	let mut events = isis.observe( ObserveConfig::default().filter( filter ) ).await.expect( "observe" );
...
async fn filter_move()
{
	...
	let mut events = isis.observe( ObserveConfig::default().filter_boxed( filter ) ).await.expect( "observe" );
